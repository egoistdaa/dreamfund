-- DreamFund refund processing functions
--
-- These functions provide:
-- 1. Atomic refund claiming with FOR UPDATE SKIP LOCKED
-- 2. Safe retry handling for Stripe API errors
-- 3. Atomic application of Stripe Refund statuses
--
-- Only service_role can execute these functions.


-- ============================================================
-- 1. Claim one refund for Stripe processing
-- ============================================================

create or replace function public.claim_next_refund()
returns table (
  refund_id uuid,
  pledge_id uuid,
  project_id uuid,
...
comment on function public.claim_next_refund() is
  'Atomically claims one eligible refund for Stripe processing. Stale claims without a Stripe Refund ID can be reclaimed after 15 minutes.';


-- ============================================================
-- 2. Release a claimed refund after a Stripe API error
-- ============================================================
...
comment on function public.release_refund_claim(uuid, text, integer) is
  'Requeues a claimed refund after a Stripe API error. After five attempts it is stopped for manual review.';


-- ============================================================
-- 3. Apply the latest Stripe Refund status atomically
-- ============================================================
...
comment on function public.apply_stripe_refund_status(
  ...
  'Atomically applies the latest Stripe Refund status to refunds, pledges, and project_settlements.';
