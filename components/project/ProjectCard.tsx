...
import {
  achievementRate, isAchieved, daysLeft, isEndingSoon, formatYen, formatManYen,
} from "@/lib/format";

/**
 * プロジェクトカード（写真主役・大判）。
 * データから状態を導出してバッジを自動表示するので、
 * 各ページは <ProjectCard project={p} /> と渡すだけ。
 */
export function ProjectCard({
  ...
  project: Project;
  topBadge?: { kind: BadgeKind; label: string }; // 行の文脈で付ける任意バッジ(例:急上昇1位)
  width?: "wide" | "full";
}) {
  ...
  return (
    <Link
      ...
      className={`group block overflow-hidden rounded-card-lg bg-white shadow-[0_12px_34px_-16px_rgba(15,23,42,.3)] transition hover:-translate-y-1 ${widthCls}`}
    >
      {/* 写真 + 重ねたタイトル */}
      <div className="relative h-[220px]">
        ...
        <div className="absolute inset-x-3 top-3 z-[2] flex flex-wrap gap-1.5">
      ...
      <div className="px-4 pb-4 pt-3.5">
        ...
        <div className="mt-2.5 flex items-baseline justify-between">
          <span className={`text-2xl font-black tracking-tight ${achieved ? "text-brand-success" : "text-brand"}`}>
            {rate}%
          </span>
          <span className="text-sm font-extrabold">
            {formatYen(project.currentAmount)}{" "}
            <span className="text-[10.5px] font-semibold text-ink-sub">/ {formatManYen(project.goalAmount)}</span>
          </span>
        </div>
        ...
        <div className="mt-2.5 flex items-center gap-3.5 border-t border-line pt-2.5">
          <span className="flex items-center gap-1.5 text-[12.5px] font-extrabold">
            <svg className="h-[15px] w-[15px] text-ink-sub" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
              <path d="M17 21v-2a4 4 0 0 0-4-4H7a4 4 0 0 0-4 4v2M9 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8" />
            </svg>
            {project.supportersCount}人が応援
          </span>
          <span className={`ml-auto flex items-center gap-1.5 text-[12.5px] font-extrabold ${endingSoon ? "text-hot" : "text-ink-sub"}`}>
