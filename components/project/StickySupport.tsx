...
/**
 * 詳細ページ下部の常時表示「支援する」バー。
 * 親指の届く位置に固定。MVPでは支援画面(ダミー)へ遷移。
 * TabBarの上に重なるよう、詳細ページではTabBarの代わりにこれを優先表示。
 */
export function StickySupport({ project }: { project: Project }) {
  const rate = achievementRate(project);
  const days = daysLeft(project.endAt);
  return (
    <div className="sticky bottom-0 z-50 border-t border-line bg-white/95 px-4 pb-[max(12px,env(safe-area-inset-bottom))] pt-3 backdrop-blur-xl">
      <div className="mb-2 flex items-center justify-between text-[11px] font-bold text-ink-sub">
        <span><span className="text-brand text-sm font-black">{rate}%</span> 達成</span>
