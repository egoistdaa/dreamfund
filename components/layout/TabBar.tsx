 ...
 *
 * 親のスマホ枠に overflow-hidden があるため、
 * React Portalを使ってdocument.body直下へ表示し、
 * 実際の画面下へ固定する。
 */
export function TabBar() {
  const Item = ({
    ...
    <Link
      href={href}
      className={`flex flex-1 flex-col items-center gap-0.5 py-1 text-[10px] font-bold ${
  ...
  const nav = (
    <nav className="fixed inset-x-0 bottom-0 z-[100] mx-auto flex w-full max-w-[390px] items-end justify-around border-t border-line bg-white/90 px-2 pb-[max(8px,env(safe-area-inset-bottom))] pt-2 backdrop-blur-xl">
      ...
      <Link href="/submit" className="flex flex-1 flex-col items-center">
        ...
        <span className="mt-1.5 text-[10px] font-extrabold text-accent">
          夢を投稿
        </span>
      </Link>
      ...
      <Item href="/mypage" label="お気に入り">
        <svg
          ...
          strokeWidth={2}
        >
          <path d="M19 14c1.5-1.5 2-3 2-5a7 7 0 1 0-7 7c2 0 3.5-.5 5-2z" />
        </svg>
      </Item>
    </nav>
  );
